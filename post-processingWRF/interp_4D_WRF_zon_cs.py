...
# SUMMARY
# This program is used to pull a zonal cross section from the WRF Output 
...
def main():
    ...
    start_point = coordinates(latitude, start_lon)
    end_point = coordinates(latitude, end_lon)
    ...
    for yr in years:        
        ...
        Data = np.zeros((np.shape(tref)[0], 150, 477))
        ...
        for filex in range(len(d03Files)):
            ...
            Data_file = np.zeros((np.shape(index_05Z)[0], 150, 477))
            ...
            for dayx in range(len(index_05Z)):
                ...
                # pull following  month of data
                    ...
                    vert       = mean(np.concatenate((first_halfv, second_halfv), axis = 0), axis=0)
                    var_windsx = mean(np.concatenate((first_halfd, second_halfd), axis = 0), axis=0)
        ...
        matfilename = 'wrf_' + str(fnum) + '_' + fourd_var + '_zon_cs_' + str(yr)
        ...
        # add vertical levels
        # vertical = wrf.to_np(merCross.vertical)
        data.update({'Z':Z})
